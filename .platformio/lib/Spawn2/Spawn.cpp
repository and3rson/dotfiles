...
void Spawn::setup() {
  this->setup(SPAWN_WIFI_SSID, SPAWN_WIFI_BSSID, SPAWN_WIFI_PASS,
              SPAWN_MQTT_SERVER, SPAWN_MQTT_PORT, SPAWN_MQTT_USER,
              SPAWN_MQTT_PASS);
}

void Spawn::setup(const char *wifiSSID, const unsigned char *wifiBSSID,
                  const char *wifiPass, const char *mqttServer,
                  uint16_t mqttPort, const char *mqttUser,
                  const char *mqttPass) {
...
bool Spawn::isConnected() {
  return this->isWiFiConnected() &&
         (!this->useMQTT || this->client.connected());
}

bool Spawn::isWiFiConnected() { return WiFi.status() == WL_CONNECTED; }
  ...
  while (!this->isConnected()) {
    ...
    if (!this->isWiFiConnected()) {
      ...
      if (this->isWiFiConnected()) {
        ...
        continue; // Restart loop since it makes no sense to proceed to MQTT
                  // connection
      }
    }
    if (this->useMQTT) {
      if (!this->client.connected()) {
        ...
        if (this->client.connect(this->deviceName, this->mqttUser,
                                 this->mqttPass, willTopic, 0, true, "OFFLINE",
                                 true)) {
