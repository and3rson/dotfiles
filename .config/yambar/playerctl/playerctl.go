...
func FetchPosition(obj dbus.BusObject) dbus.Variant {
	// TODO: return err
	result, err := obj.GetProperty("org.mpris.MediaPlayer2.Player.Position")
	if err != nil {
		panic("failed to fetch playback status")
	}
	return result
...
func GetPositionFromPosition(rawPosition dbus.Variant) string {
	var position int64
	rawPosition.Store(&position)
	position = position / 1e6
	return fmt.Sprintf("%02d:%02d", position/60, position%60)
}
...
func (p *PlayerCtl) update(obj dbus.BusObject) {
	...
	if controlled {
		...
		p.isPlaying = GetPlayingFromPlaybackStatus(FetchPlaybackStatus(obj))
		p.position = GetPositionFromPosition(FetchPosition(obj))
	} else {
		p.info = Info{
			title:    "No players",
			duration: "",
